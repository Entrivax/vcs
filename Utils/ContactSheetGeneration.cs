        ...
        public static Task<Image<Rgba32>> Generate(FileConfig file, TimeSpan[] timestamps, Image[] thumbnails, IReadOnlyFontCollection fontCollection, IProgress<ContactSheetGenerationProgressReport>? progress = null)
        {
            ...
            return Task.Run(() =>
            {
                ...
                image.Mutate(x =>
                {
                    ...
                    $"{TimestampUtils.SecondsToTimestamp((float)file.MediaInfo.Duration.TotalSeconds)}, {FileSizeUtils.GetFileSizeString((long)(file.MediaInfo.Format.BitRate * file.MediaInfo.Format.Duration.TotalSeconds))}\n" +
